...
|---|---|---|
| Backend | `.github/instructions/backend.instructions.md` | `ms-*/src/main/java/**/*.java` |
| Frontend | `.github/instructions/frontend.instructions.md` | `api-gateway/**` · `frontend/**` |
| Tests | `.github/instructions/tests.instructions.md` | `ms-*/src/test/**/*.java` · `frontend/src/__tests__/**` |
...
|---------|-----------|---------------------|
| **Evento** (`event`) | Obra de teatro publicada para venta | Función, show |
| **Aforo** (`capacity`) | Cantidad total de entradas disponibles por Evento | Cupo, stock |
| **Tier** (`tier`) | Nivel de precio por Evento (`VIP`, `General`, `Early Bird`) | Categoría libre |
| **Reserva** (`reservation`) | Bloqueo temporal de entradas antes del pago | Pre-compra |
| **Ticket** (`ticket`) | Comprobante confirmado después de pago exitoso | Boleta |
| **Pago Simulado** (`mock payment`) | Resultado de autorización en entorno de desarrollo | Pago real |
| **Expiración** (`expiration`) | Liberación automática de reservas vencidas/fallidas | Timeout invisible |
| **Notificación** (`notification`) | Mensaje enviado al comprador tras eventos de negocio | Aviso genérico |
| **RabbitMQ Event** | Mensaje de dominio publicado/consumido entre microservicios | Log interno |
| **Administrador** | Rol con permisos de gestión de catálogo | Superusuario |
| `created_at` | Timestamp de creación en UTC | Fecha alta |
| `updated_at` | Timestamp de última actualización en UTC | Fecha modificación |

**Reglas:** usar nombres de dominio del sistema de ticketing, conservar `created_at` y `updated_at` en UTC, y mantener contratos entre microservicios por eventos RabbitMQ.
