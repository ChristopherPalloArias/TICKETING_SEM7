...
2. **[FASE 2 — Paralelo]** al aprobar la spec:
   - `Backend Developer` → implementa `ms-*/src/main/java/`
   - `Frontend Developer` → implementa `frontend/` (si aplica)
   - `Database Agent` → si hay cambios de esquema en la spec
3. **[FASE 3 — Paralelo]** al completar implementación:
   - `Test Engineer Backend` → genera `ms-*/src/test/java/`
   - `Test Engineer Frontend` → genera `frontend/src/__tests__/` (si aplica)
